...
const Format = ({ category, breadcrumbText, pageTitle }) => {
    ...
    useEffect(() => {
        axios.get('https://sneakers-backend-1.onrender.com/posts')
            .then(response => {
                ...
                setFilteredPosts(filteredPosts); // Initial set to show all products
        ...
        <div className="container collection pb-10">
            <div className="text-center pt-6 pb-4 lg:pt-10">
                <div className='border-b pb-6'>
                    <a href='/' className="text-center lg:text-start text-sm lg:text-base pl-4">{breadcrumbText}</a>
                    {/* You can change the page title dynamically */}
                    <h1 className="text-xl lg:text-3xl filter-tittle-3 font-bold mt-2 text-center lg:text-start pl-4">{pageTitle}</h1>
                </div>
                ...
                <div className="container grid grid-cols-2  h-full  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 mx-auto px-4">
                    {loading ? (
                        ...
                        filteredPosts.map(product => (
                            <div key={product._id} className="relative bg-white rounded-sm shadow-md">
                                ...
                                <div className="ps-2 pb-1">
                                    ...
                                    <h2 className="   product-card__title text-start pt-1 font-semibold text-gray-900">
                                        {product.title}
                                    </h2>
                                    <div>
                        ...
                        <div className="text-center col-span-full py-12">
                            <p className="text-4xl text-gray-500 mb-6">🔍</p>
                            <p className="text-xl font-semibold text-gray-700">No products match your search criteria.</p>
                            <p className="text-md text-gray-500 mt-2">Please refine your filters or search terms for better results.</p>
                        </div>
                    )
                    }
                </div>
