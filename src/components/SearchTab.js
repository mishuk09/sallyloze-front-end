...
const SearchTab = ({ closeSearch }) => {
    ...
    useEffect(() => {
        const fetchDefaultItems = async () => {
            try {
                const response = await fetch('https://sneakers-backend-1.onrender.com/posts');
    ...
    const handleSearchChange = async (event) => {
        ...
        if (searchQuery.trim()) {
            try {
                const response = await fetch(`https://sneakers-backend-1.onrender.com/posts?q=${searchQuery}`);
                const data = await response.json();
